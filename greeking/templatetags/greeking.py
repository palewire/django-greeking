...
def pangram(parser, token):
    ...
    ``language`` is the two-letter abbreviation of the desired language.
...
def jabberwocky(parser, token):
    """
    Prints paragraphs from Lewis Carroll's poem Jabberwocky for greeking in templates.
    ...
    ``count`` is a number (or variable) containing the number of paragraphs or
    to pull (default is 7, which prints the entire poem).
